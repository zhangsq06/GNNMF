...
def Acoo_value(seqs,K12,K1,K2):    
    ...
    return weight
    #######################jaccard edge
def Jacc_value(Ks1,Ks2):    

    jacc = len(set(Ks1) & set(Ks2))/len(set(Ks1) | set(Ks2))
    return jacc
    ######################
def Acoo(seqs,word_freq,word_vocab):
    ...
    jacc_rows=np.random.rand(len(kmers))
    jacc_cols=np.random.rand(len(kmers))
    jacc_weights=np.random.rand(len(kmers))
    ...
    for t in range(len(kmers)):
        ...
        jaccard = Jacc_value(set(kmer_seqs[kmers[t][0]]),set(kmer_seqs[kmers[t][1]]))
        if K12>0:
        ...
        jacc_weights[t] = jaccard
        jacc_rows[t]= rows_cols[t][0]
        jacc_cols[t] = rows_cols[t][1]

    node_size = len(keys)
    weights = standardization(weights) #
    jacc_weights = standardization(jacc_weights) #
    adj = scipy.sparse.csr_matrix((weights, (list(rows), list(cols))),shape=(node_size, node_size))
    jacc_adj = scipy.sparse.csr_matrix((jacc_weights, (list(rows), list(cols))),shape=(node_size, node_size))
    # adj = adj.T+adj
    Acoo = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)
    jacc_Acoo = jacc_adj + jacc_adj.T.multiply(jacc_adj.T > jacc_adj) - jacc_adj.multiply(jacc_adj.T > jacc_adj)
    return Acoo,jacc_Acoo
#################calculate the inclusive, coexisting and similarity matrices############################
def generateAdjs(tfid,Kmers=5):
    _, _, test_seqs = load_encode_test(tfid)
    _, _, train_seqs = load_encode_train(tfid)
    ...
    adj_coo,adj_jacc= Acoo(seqs,word_freq,word_vocab)
    return adj_inclu, adj_sim,adj_coo,adj_jacc, labels

if __name__=='__main__':    
    start_time=time.time()
    adj_inclu, adj_sim,adj_coo,adj_jacc, labels= generateAdjs(args.dataset,Kmers=5)
    end_time=time.time()
    total_time=np.array(end_time-start_time)
    print(adj_inclu.toarray().shape, adj_sim.toarray().shape, adj_coo.toarray().shape,adj_jacc.toarray().shape,len(labels))
