...
def dinucshuffle(sequence):
	b = [sequence[i:i+1] for i in range(0, len(sequence), 1)]
	np.random.shuffle(b)
...
def load_encode_train(tfid, limit=5000,):
	seq_suffix =".seq"
	filename = "../../multiple_mmgraph2/data/encode_101M/%s_AC%s" % (tfid, seq_suffix)
	pos_sequences=[]
	neg_sequences=[]
	sequences=[]
	targets=[]
...
def load_encode_test(tfid):
	seq_suffix =".seq"
	filename = "../../multiple_mmgraph2/data/encode_101M/%s_B%s" % (tfid, seq_suffix)
	pos_sequences=[]
	neg_sequences=[]
	sequences=[]
	targets=[]
